...
public:
  ...
  void CombineOrderBook(const OrderBook<T>& source)
  {
	  vector<Order> bidStack_source = source.GetBidStack();
	  vector<Order> offerStack_source = source.GetOfferStack();
	  int m = bidStack_source.size();
	  for (int i = 0; i < m; i++)
	  {
		  bidStack.push_back(bidStack_source[i]);
		  offerStack.push_back(offerStack_source[i]);
	  }
	  auto smaller_f = [](Order order1, Order order2) -> bool {
		  return (order1.GetPrice() < order2.GetPrice());
	  };
	  auto greater_f = [](Order order1, Order order2) -> bool {
		  return (order1.GetPrice() > order2.GetPrice());
	  };
	  sort(bidStack.begin(), bidStack.end(), greater_f);
	  sort(offerStack.begin(), offerStack.end(), smaller_f);
  }
private:
  T product;
